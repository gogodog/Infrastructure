...
<body>
<div class="title"><h2>权限管理</h2></div>
<div class="query">
...
$('.pagination').pagination(${list.totalResults},{
	callback: function(page){
		window.location.href = "/user/list?currentPage="+ (page+1) +"&name="+ $("#name").val();
	},
...
$('.confirm').on("click" , function(){window.location.href = "/user/list?currentPage=1&name="+ $("#name").val()});
...
$.each($("#list").children(), function(){
 	$(this).on("click" , function(){
 		...
 		var fuc = "'" + "/user/del?userId="+$(this).children(":first").html() + "'";
 		$(".del").attr("onclick","delline("+ fuc +")");
 		fuc = "'" + "/user/edit?userId="+$(this).children(":first").html() + "'";
